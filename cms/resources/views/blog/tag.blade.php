...
@section('seo_title', '#' . $tag->name . ' | Blog | CYT Comunicaciones')
@section('seo_description', 'Artículos etiquetados con ' . $tag->name . ' - Blog de CYT Comunicaciones.')
...
<section class="relative overflow-hidden px-6 py-20"
    ...
    <div class="relative mx-auto max-w-7xl">
        <nav class="mb-5 flex flex-wrap items-center gap-1.5 text-xs font-semibold text-muted/70">
            ...
            <span class="text-muted/60">#{{ $tag->name }}</span>
        </nav>
            ...
            <div class="min-w-0 flex-1">
                @if($posts->isEmpty())
                    <div class="flex flex-col items-center justify-center py-24 text-center">
                        <span class="material-symbols-outlined text-6xl text-brand/20">label_off</span>
                        <p class="mt-4 text-lg font-semibold text-[#5a4e6e]">No hay artículos con esta etiqueta.</p>
                                        ...
                                        <div class="mb-3 flex flex-wrap gap-1.5">
                                            @foreach($post->categories as $cat)
                                                <a href="{{ route('blog.category', $cat->slug) }}"
                                                   class="rounded-full px-2.5 py-0.5 text-xs font-bold uppercase tracking-wide text-brand transition hover:bg-brand/15"
            ...
            <aside class="w-full shrink-0 space-y-6 lg:w-64">
                ...
                @if(isset($tags) && $tags->isNotEmpty())
                    <div class="rounded-[2rem] border border-brand/10 bg-white p-6 shadow-sm">
                        <h3 class="mb-4 text-xs font-extrabold uppercase tracking-[0.28em] text-[#9e92b8]">Otras etiquetas</h3>
                        <div class="flex flex-wrap gap-2">
                            @foreach($tags->where('slug', '!=', $tag->slug)->take(20) as $t)
                                @if($t->posts_count > 0)
